-- Notify the request creator when their request is approved by an admin.
-- Previously only designers were notified. Now the teacher who created
-- the request also gets "Your request was approved: [title]".
...
begin
  ...
  -- admin approved -> designers + creator
    ...
    -- notify the creator that their request was approved
    if new.created_by is not null
       and new.created_by <> coalesce(v_actor, v_nil) then
      insert into public.notifications (recipient_id, actor_id, type, request_id, body)
      values (new.created_by, v_actor, 'request_approved', new.id,
              'Your request was approved: ' || v_title);
    end if;
