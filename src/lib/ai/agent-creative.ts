...
- LOOK at each brand asset image carefully. These contain SCHOOL BRANDING ELEMENTS — not rigid "header bars" or "footer bars".
- EXTRACT the useful information from these images:
  - Logo: the school crest/emblem
  - School name and tagline (e.g., "Sri Sri Ravishankar Vidya Mandir — CBSE Affiliation #830328")
  - Contact details: phone number, website, address
  - Any affiliations, mottos, or certifications shown
- You are a CREATIVE DIRECTOR — place these branding elements wherever makes sense for YOUR design:
  - A modern minimal design might place the logo small in a corner with the school name as subtle text
  - A formal design might have a full branded bar at the top
  - Contact info could be a slim strip at the bottom, icons scattered along an edge, or grouped on the closing page of a carousel
  - The logo could float over the hero image, sit in a corner badge, or anchor a header bar
- Do NOT blindly copy the header/footer images as rigid bars. Instead, extract their content and integrate it naturally into your poster design.
- In the "brandingPlacement" field, describe exactly WHERE and HOW you're placing each branding element (logo, school name, contact info) and what style treatment they get.
- For CAROUSEL: branding placement MUST be consistent across all pages (same position, same style).

ASSET SELECTION:
- Logo: pick the BEST logo from available options. Specify its exact storage_path in selectedAssets.
- Header/Footer: still specify their storage_paths in selectedAssets so the image model receives them as reference for extracting content. Set to null if a school has none uploaded.
...
NO DUPLICATE PHOTOS: Each uploaded photo can appear on ONLY ONE page. Do NOT assign the same image path to multiple pages. Every photo should be unique to its page.
...
4. Check NO photo path appears on more than one page — every photo is unique to one page.
...
Return ONLY valid JSON matching this schema:
{
  "variations": [{
    ...
    "logoPlacement": { "position": "string", "size": "string", "style": "string" },
    "brandingPlacement": {
      "schoolName": "where and how the school name text appears (e.g., 'top-center in white Montserrat 14px' or 'integrated into a header bar')",
      "contactInfo": "where and how contact details appear (e.g., 'bottom strip with phone icon + number + website' or 'closing page only')",
      "affiliations": "where certifications/CBSE number appear (e.g., 'below school name in 10px' or 'omitted for clean look')",
      "consistency": "for carousel — describe how branding is identical on every page"
    },
    "headerFooter": { "headerStyle": "describe how the header area looks — could be a full bar, a subtle top edge, or just a logo+name combo", "footerStyle": "describe how the footer/contact area looks — could be a bar, icons, or minimal text" },
...
Create 1 creative direction brief. Make it the strongest possible direction for this theme.`;
  ...
  console.log(`[Agent2] Brand assets: ${sampleAssets.length} samples, ${otherAssets.length} other (${otherAssets.map((a) => a.assetType).join(", ")})`);
  ...
  if (sampleAssets.length > 0) {
    ...
    console.log(`[Agent2] Selected ${selected.length}/${sampleAssets.length} sample posters: ${selected.map((s) => s.label ?? s.storagePath.split("/").pop()).join(", ")}`);
    const withUrls = selected.filter((s) => !!s.signedUrl);
    if (withUrls.length < selected.length) {
      console.warn(`[Agent2] ${selected.length - withUrls.length} samples have no signed URL — skipped`);
    }
