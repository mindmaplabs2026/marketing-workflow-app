...
- IMPORTANT: Each carousel page is generated by a SEPARATE AI image model call, so each page MUST be self-contained:
  - Each page object MUST have its own "creativeVision" field (50-100 words) — a rich visual description of THAT specific page's layout, hero content, text placement, and mood.
  - Each page object MUST have its own "designPrompt" field — a concise technical prompt for THAT page's image generation.
  - The top-level "creativeVision" field should ONLY contain the shared visual consistency rules (typography, colors, borders, background pattern, header/footer style) that apply to ALL pages.
  - The top-level "designPrompt" field should be a brief overall summary.
- Each page's "description" should be a detailed visual description, and page-level selectedImages should specify which uploaded photos go on THAT page (not all photos on every page).
...
Return ONLY valid JSON matching this schema:
{
  "variations": [{
    ...
    "layout": { "type": "single|carousel", "pages": [{ "pageIndex": 1, "description": "visual description of this page", "selectedImages": [...], "textOverlays": [{ "text": "...", "position": "...", "style": "..." }], "creativeVision": "FOR CAROUSEL ONLY — rich per-page visual narrative (50-100 words) describing this specific page's layout, hero content, composition, and mood", "designPrompt": "FOR CAROUSEL ONLY — concise per-page technical prompt for the image model" }] },
    "logoPlacement": { "position": "string", "size": "string", "style": "string" },
    "headerFooter": { "headerStyle": "string", "footerStyle": "string" },
    "schoolAssetUsage": { "useUniform": true/false, "uniformNotes": "...", "useInfrastructure": true/false, "infrastructureNotes": "..." },
    "creativeVision": "string — For SINGLE posters: THIS IS THE MOST IMPORTANT FIELD. Write a rich, vivid, detailed creative narrative describing the poster top-to-bottom (min 150 words). For CAROUSELS: Write ONLY the shared visual consistency rules — typography (font family, sizes, weights), color application, background texture/pattern, border/frame treatment, header/footer appearance, decorative elements. Each page's individual vision goes in layout.pages[].creativeVision instead.",
